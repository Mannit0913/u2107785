...
london_data2<-read.csv("H:/data science/london_data4.csv")
...
#investigating distribution of price to find it is right skewed
ggplot(london_data_clean, aes(x = price)) +
...
##taking logarithms of price, capacity variables, and distance following literature
...
london_data$logdistancetube<-log(london_data$distance_to_tube)
london_data$logprice<-log(london_data$price)

##model building and validation
#updating 'clean' data set

#specifying variables of interest
#numerical variables
num_vars <- c("review_scores_location", "logbedrooms", "logbeds","logbathrooms","distance_to_tube", "logaccommodates", "logdistancecentre", "excessbeds", "important_words_score")

# Keep only numeric variables
num_vars <- num_vars[sapply(london_data_clean[num_vars], is.numeric)]
...
summary(model1)
#compare against log price specification
